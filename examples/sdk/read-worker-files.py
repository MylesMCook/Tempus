...
report = []
assert result.get("diagnosticVersion", 1) in (1, 2, 3, 4)
...
if result.get("diagnosticVersion", 1) >= 3:
    expected_names = {"recipient-clock", "recipient-dst"}
    if result["diagnosticVersion"] == 4:
        expected_names.add("recipient-lowercase")
        assert result["rejectedConditions"] == ["pending", "assuming", "provided"]
    assert set(result["reminderFiles"]) == expected_names
    for name, text in result["reminderFiles"].items():
        events = icalendar.Calendar.from_ical(text).walk("VEVENT")
        assert len(events) == 1
        event = events[0]
        assert str(event["SUMMARY"]) == ("Buy apples for mom" if name == "recipient-lowercase" else "Buy apples for Sam")
