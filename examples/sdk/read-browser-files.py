...
assert result.get("journeyVersion", 1) in (1, 2)
...
for run in result["runs"]:
    ...
    if result.get("journeyVersion") == 2:
        expected_names.add(f'{run["browser"]}-{run["width"]}-recipient-lowercase.ics')
    assert len(run["files"]) == len(expected_names) and {file["filename"] for file in run["files"]} == expected_names
    for file in run["files"]:
        raw = (folder / file["filename"]).read_bytes()
        assert hashlib.sha256(raw).hexdigest() == file["sha256"]
        calendar = icalendar.Calendar.from_ical(raw)
        if file["filename"].endswith("-recipient-lowercase.ics"):
            events = calendar.walk("VEVENT")
            assert len(events) == 1
            event = events[0]
            assert str(event["SUMMARY"]) == "Buy apples for mom"
            ...
            assert observed == "2026-11-01T07:30:00+00:00"
