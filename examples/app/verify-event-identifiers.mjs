...
try {
  for (const width of [320, 1280])
    for (const [id, expression, title, choices, count] of [
      ...
      [
        "quantity-list",
        "Buy 3 apples and 2 pears on 11/01/2026 at 1:30am for 30 minutes",
        "Buy 3 apples and 2 pears",
        [/^Use “Buy 3 apples and 2 pears”$/, /November 1, 2026/, /UTC-06:00/],
        1,
      ],
      [
        "recipient",
      ...
      [
        "recipient-lowercase",
        "Buy apples for mom on 11/01/2026 at 1:30am for 30 minutes",
        "Buy apples for mom",
        [/^Use “Buy apples for mom”$/, /November 1, 2026/, /UTC-06:00/],
        1,
      ],
      [
        "clock-first",
      ...
      await page.addInitScript(() => {
        ...
        const click = Reflect.get(HTMLAnchorElement.prototype, "click");
      ...
      try {
        ...
        await input.fill(
          id === "recipient-lowercase"
            ? expression.replace("mom", "dad")
        ...
        if (id !== "schedule") {
          await page
            .getByRole("button", {
              name:
                id === "recipient-lowercase"
                  ? /^Use “Buy apples for dad”$/
                  : id === "recipient" || id === "clock-first"
                    ? /^Use “Buy apples for Jo”$/
                    : id === "invoice"
                      ? /November 1, 2026/
                      : id === "quantity" || id === "quantity-list"
                        ? id === "quantity-list"
                          ? /^Use “Buy 4 apples and 2 pears”$/
                          : /^Use “Buy 4 apples”$/
                        : /^Use “Room 4”$/,
            })
            .waitFor();
        ...
        assert.equal(await page.locator("#calendar-export-toggle").count(), 0);
        if (id === "recipient-lowercase") {
          for (const condition of ["pending", "assuming", "provided"]) {
            const conditional = `Buy apples for mom ${condition} tomorrow at noon`;
            await input.fill(conditional);
            await page.locator("#calculation-error").waitFor();
            assert.equal(await input.inputValue(), conditional);
        ...
        if (id === "quantity-list") {
          for (const invalid of [
            ...
            assert.equal(await page.locator("#calculation-error button").count(), 0);
            assert.equal(await page.locator("#calendar-export-toggle").count(), 0);
          }
        }
        ...
        await page
          ...
          .catch(() => {});
        writeFileSync(join(output, `${String(id)}-${width}-failed.html`), await page.content());
        throw error;
      } finally {
