...
function quantityLabelProposal(text: string) {
  ...
  for (;;) {
    ...
    const nextQuantity = /^(?:[1-9]\d*|one|two|three|four|five|six|seven|eight|nine|ten)\s+/i.exec(
      target.slice(boundary),
    );
    const continues = /\s+and$/i.test(object) && nextQuantity;
    const item = continues ? object.replace(/\s+and$/i, "") : object;
    if (
      !validEventLabel(item) ||
      /\bthen\b|\band$|\band\s+(?:call|email|text|visit|pay|buy|send|submit|pick up)\b/i.test(item)
    )
      return undefined;
    if (continues) {
...
export function interpretDate(
  ...
  if (
    /\b(?:actually|instead|unless|if|or|reschedule|pending|assuming|provided)\b/i.test(text) ||
  ...
  if (reminder || actionPattern.test(text)) {
    ...
    if (labelTarget && validEventLabel(labelTarget)) {
      ...
      if (
        connector &&
        /^\p{L}[\p{L}'’-]*(?:\s+\p{L}[\p{L}'’-]*){0,2}$/u.test(recipient) &&
        !/^(?:a|an|the|ever|forever|life|lifetime|long|short|some|any|several|many|few|awhile)\b|\b(?:time|eternity|future|maybe|perhaps|probably|possibly|tentatively|optionally|unless|if|then)\b/i.test(
          recipient,
        )
      ) {
