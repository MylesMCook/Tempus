...
- Whole months and years clamp at each step. January 31 plus one month plus one month is March 28. Fractional months/years are rejected; write the intended whole months or days.
...
**Migration from v1:** remove `preserveDayOfMonth`; it now returns an explicit error. `settings` and heuristic `meta` were replaced with calculation inputs and the executed trace. Calendar arithmetic now uses the requested timezone and written order. Replace fractional month/year expressions. This is an intentional semantic change; old clients should check `engineVersion`.
