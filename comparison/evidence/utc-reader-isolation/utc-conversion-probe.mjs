...
const text = [
  "BEGIN:VTIMEZONE", "TZID:Probe/Seasonal",
  "BEGIN:STANDARD", "DTSTART:20251102T020000", "TZOFFSETFROM:-0500", "TZOFFSETTO:-0600", "END:STANDARD",
  "BEGIN:DAYLIGHT", "DTSTART:20260308T020000", "TZOFFSETFROM:-0600", "TZOFFSETTO:-0500", "END:DAYLIGHT",
  "BEGIN:STANDARD", "DTSTART:20261101T020000", "TZOFFSETFROM:-0500", "TZOFFSETTO:-0600", "END:STANDARD",
  "END:VTIMEZONE", "",
].join("\r\n");
const checkpoints = [
...
const zone = new ICAL.Timezone({ component: new ICAL.Component(ICAL.parse(text)), tzid: "Probe/Seasonal" });
const rows = checkpoints.map(([instant, expected, offset]) => {
  ...
  return { instant, expected, offset, observed, matches: expected === observed, returnedInstant: converted.toJSDate().toISOString(), utcControl: utc.convertToZone(ICAL.Timezone.utcTimezone).toString() };
});
const hash = (data) => createHash("sha256").update(data).digest("hex");
const reader = new URL("../../node_modules/ical.js/", import.meta.url);
const report = {
  probe: "authored-utc-to-local-v1", reader: "ical.js",
  ...
  fixtureSha256: hash(text), rows,
  conforms: rows.every((row) => row.matches && row.utcControl === row.instant),
  scope: "Nine authored clocks in a single finite timezone fixture. No Tempus generator, calendar-client import, general reader certification or independent-user evaluation.",
};
writeFileSync(join(output, "timezone.ics"), text);
writeFileSync(join(output, "report.json"), JSON.stringify(report, null, 2) + "\n");
console.log(JSON.stringify({ conforms: report.conforms, matches: rows.filter((r) => r.matches).length, total: rows.length }));
