...
for item in files:
    ...
    assert len({str(event["UID"]) for event in events}) == 1000
    dates = []
    for event in events:
...
print(json.dumps({"files": results, "scope": "Separate Python readers verify complete files, dates, durations, titles and unique UIDs. SDK first date has a fixed oracle; app checks continuity, not its captured reference. No actual client import."}, indent=2))
