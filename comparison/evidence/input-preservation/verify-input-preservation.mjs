...
assert.ok(playwrightEntry && outputArg, "Usage: node verify-input-preservation.mjs PLAYWRIGHT-ENTRY NEW-OUTPUT");
...
const report = {
  ...
  original,
  runs: [],
  scope: "Authored desktop text insertion and explicit editing; not physical-device, native clipboard, calendar import or independent-user evidence.",
};
try {
  for (const name of ["chromium", "firefox", "webkit"]) {
    ...
    try {
      for (const width of [320, 1280]) {
        ...
        const run = { browser: name, version: browser.version(), width, status: "running", errors: [] };
        ...
        try {
          ...
          await page.screenshot({ path: join(output, `${name}-${width}-length-error.png`), fullPage: true });
          ...
          await page.screenshot({ path: join(output, `${name}-${width}-failure.png`), fullPage: true });
          throw error;
        } finally {
          await page.close();
        }
