...
Import the optional calendar entry separately. These helpers return file text. Your app handles review, download and any calendar write. New integrations should use `prepareCalendar` from the [quickstart](README.md).
...
| `@tempus-date/core/calendar` | `prepareCalendar`, `calendarContextKey`, `prepareCalendarFile`, `resolveRecurringExport`, `prepareRecurringCalendarFile`, `retainOccurrenceDecisions` | `CalendarPreparation`, `CalendarMetadata`, `PreparedSchedule`, `CalendarFile`, `CalendarFileFailureCode` |
